...
ここで PaaS の魔法が本当に起こるのです。
HTML などのちょっとしたコードを変更して、それを heroku リモートにプッシュすると、
heroku はそのコードを格納して、新しい不変的なコンテナを作成してデプロイしてくれます。
